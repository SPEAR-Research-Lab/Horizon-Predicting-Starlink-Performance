...
def get_cf_best_servers_query(date_from: str, date_to: str) -> str:
    ...
    SELECT DISTINCT
      cs.clientCity AS client_city,
      cs.clientCountry AS client_country_code,
      cs.serverPoP AS server_airport_code,
...
def get_ndt_best_servers_query(date_from: str, date_to: str) -> str:
    ...
    latency_thresholds AS (
      SELECT DISTINCT
        client_city,
        client_country_code,
        PERCENTILE_CONT(download_latency_ms, 0.01) OVER (PARTITION BY client_city, client_country_code) AS download_latency_threshold_ms,
        PERCENTILE_CONT(upload_latency_ms, 0.01) OVER (PARTITION BY client_city, client_country_code) AS upload_latency_threshold_ms
    ...
    SELECT DISTINCT
      s.client_city,
      s.client_country_code,
      s.server_city,
      s.server_country_code,
    FROM server_for_client s
    JOIN latency_thresholds lt
      ON s.client_city = lt.client_city AND s.client_country_code = lt.client_country_code
    WHERE
