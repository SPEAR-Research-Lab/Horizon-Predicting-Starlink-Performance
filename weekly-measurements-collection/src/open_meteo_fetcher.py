...
class OpenMeteoFetcher:
    ...
    def fetch_and_save_forecast(
        self, latitude: float, longitude: float, file_name: str, past_days: int = 3, forecast_days: int = 3
    ) -> None:
    ...
    def _fetch_missing_weather_data(self, min_max_date_map: dict[tuple[str, str], dict]) -> None:
        for (city, country), date_range in min_max_date_map.items():
            ...
            self.fetch_and_save_forecast(
                ...
                past_days=(datetime.now(timezone.utc).date() - required_start_date).days,
                forecast_days=15,
            )
