...
from utils import filter_df_by_min_measurements, iso2_to_iso3
...
def compute_metric_difference(
    ...
    before_metrics = compute_country_metric(
        df_before_filtered, measurement_column, metric, country_column
    )
    after_metrics = compute_country_metric(
        df_after_filtered, measurement_column, metric, country_column
    )
...
def plot_metric_comparison_map(
    diff_df: pd.DataFrame, measurement_name: str, colorscale="turbo"
) -> go.Figure:
    """
...
def plot_before_after_maps(
    diff_df: pd.DataFrame, measurement_name: str, metric: Metric, colorscale="turbo"
) -> go.Figure:
    """
...
def plot_filtering_percentage(
    df_all: pd.DataFrame, df_filtered: pd.DataFrame, color_scale="turbo"
) -> go.Figure:
    """
    ...
    def calculate_percentages(
        df_all_split: pd.DataFrame, df_filtered_split: pd.DataFrame
    ) -> pd.DataFrame:
        if df_all_split is None or df_all_split.empty:
            raise ValueError("Empty dataframes.")

        all_counts = (
            df_all_split.groupby("client_country_code").size().reset_index(name="total")
        )
        filtered_counts = (
            df_filtered_split.groupby("client_country_code")
            .size()
