...
def get_weather_file_name(lat: float, lon: float) -> str:
    ...
    return f"weather_lat{str(lat).replace('.', '_')}_lon{str(lon).replace('.', '_')}.csv"
...
def get_and_maybe_fetch_openmeteo_data(lat, lon, start_date, end_date):
    ...
    if os.path.exists(weather_data_dir / weather_file):
        ...
        if start_date <= om_df['datetime'].min().date() and end_date >= om_df['datetime'].max().date():
            return om_df

    return fetch_and_save_historical(lat, lon, start_date, end_date, weather_file, weather_data_dir)

def fetch_and_save_historical(lat, lon, start_date, end_date, weather_file, weather_data_dir):
