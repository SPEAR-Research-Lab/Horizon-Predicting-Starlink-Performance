...
def cross_validate_location(lat: float, lon: float, loc_df: pd.DataFrame, start_date, end_date) -> Optional[dict]:
    ...
    for feat in feature_to_y_label.keys():
        ...
        if knmi_name is None or om_name is None:
            raise RuntimeError(f"Could not find columns for feature {feat} in merged data for {station_name}")
    ...
    return {"station": station_name, "lat": lat, "lon": lon, "metrics": metrics, "merged_df": merged}
...
def main() -> None:
    ...
    for lat, lon in locations[['lat', 'lon']].itertuples(index=False):
        loc_df = df_filtered[(df_filtered['lat'] == lat) & (df_filtered['lon'] == lon)].copy()
        res = cross_validate_location(lat, lon, loc_df, start_time, end_time)
        results.append(res)
        if res is not None:
            summary.append({"station": res['station'], "lat": lat, "lon": lon, "metrics": res['metrics']})

    summary_df = pd.DataFrame(summary)
    summary_df.to_json(output_dir / "netherlands_validation_summary.json", orient="records", indent=4)
