...
def get_starlink_countries() -> list[str]:
    ...
    ul_element = wait.until(
        EC.presence_of_element_located(
            (By.CSS_SELECTOR, "ul[role='listbox'].MuiList-root.MuiList-padding.MuiMenu-list")
        )
    )

    li_elements = ul_element.find_elements(By.TAG_NAME, "li")
    countries = [li.get_attribute("data-value") for li in li_elements if li.get_attribute("data-value")]
