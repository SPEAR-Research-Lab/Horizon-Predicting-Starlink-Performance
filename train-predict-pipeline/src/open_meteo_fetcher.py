...
from typing import MutableMapping, Any, cast, Union
from custom_types import HistoricalParams, ForecastParams
from constants import weather_data_dir, logger
...
class OpenMeteoFetcher:
	...
	def _fetch(self, url: str, params: Union[HistoricalParams, ForecastParams]) -> pd.DataFrame:
		...
		hourly_data: dict[str, Any] = {"date": pd.date_range(
	...
	def _fetch_missing_weather_data(self, min_max_date_map: dict[tuple[str, str], dict]) -> None:
		for (city, country), date_range in min_max_date_map.items():
			...
			if not file_path.exists():
				logger.info(f"Weather file not found for {city}, {country}. Fetching data from {required_start_date} to {required_end_date}")
				...
				try:
					...
					if has_gaps:
						logger.info(f"Weather data for {city}, {country} has gaps ({distinct_dates_in_file}/{expected_days_in_range} days). Refetching entire range.")
					elif needs_earlier_data and needs_later_data:
						logger.info(f"Extending weather data for {city}, {country} in both directions: {fetch_start_date} to {fetch_end_date}")
					elif needs_earlier_data:
						logger.info(f"Fetching earlier weather data for {city}, {country}: {fetch_start_date} to {existing_start_date - timedelta(days=1)}")
						fetch_end_date = existing_start_date - timedelta(days=1)
					elif needs_later_data:
						logger.info(f"Fetching later weather data for {city}, {country}: {existing_end_date + timedelta(days=1)} to {fetch_end_date}")
					...
					logger.error(f"Error checking existing weather file for {city}, {country}: {e}")
	...
	def fetch_weather_data_for_dataframe(self, df: pd.DataFrame) -> None:
		...
		self._fetch_missing_weather_data(min_max_date_map)
