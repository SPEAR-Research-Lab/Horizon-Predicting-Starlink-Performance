...
def enrich_data_directory(src_dir: str, dst_dir: str, with_sat_density: bool) -> None:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Enrich raw measurement data with weather, distance, and satellite density features."
    )
    parser.add_argument(
        "--src",
        "-s",
    ...
    parser.add_argument(
        ...
        "-d",
        type=str,
        help="Destination directory or file path for enriched CSV output",
    )
    parser.add_argument(
        "--with-sat-density",
        action="store_true",
        help="Whether to enrich data with satellite density features (requires additional API calls)",
    )
    ...
    src_dir = args.src
    dst_dir = args.dst
    with_sat_density = args.with_sat_density

    if not src_dir:
        src_dir = input("Enter source directory/file path: ").strip()
    if not dst_dir:
        dst_dir = input("Enter destination directory/file path: ").strip()

    if not os.path.exists(src_dir):
        logger.error(f"Source path does not exist: {src_dir}")
        exit(1)
    ...
    if os.path.isdir(src_dir):
        enrich_data_directory(src_dir, dst_dir, with_sat_density)
    else:
        enrich_data_file(src_dir, dst_dir, with_sat_density)
