...
def enrich_data_file(src_file_path: str, dst_file_path: str,
                     with_sat_density: bool = False, coords_csv: str = None, sat_dir: str = None) -> None:
    ...
    if with_sat_density:
        logger.info(f"Satellite density extraction...")
        df = enrich_with_sat_density(df, coords_csv, sat_dir)
...
if __name__ == "__main__":
    ...
    # If flags not provided, ask interactively
    if not src_dir:
    ...
    if os.path.isdir(src_dir):
        ...
        enrich_data_file(src_dir, dst_dir)
