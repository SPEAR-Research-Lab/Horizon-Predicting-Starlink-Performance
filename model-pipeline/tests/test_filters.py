...
from src.anomaly.filters import (filter_incomplete_measurements,
...
def test_filter_incomplete_measurements(sample_data):
    df = sample_data.copy()
...
def test_percentile_filtering(sample_data):
    result = filter_outliers_percentile(sample_data, 0.75, ["download_latency_ms"])
    assert len(result) <= len(sample_data)
    assert "download_latency_ms" in result.columns


def test_directional_mad_filtering(sample_data):
    result = filter_outliers_directional_mad(sample_data, 2.5, ["download_latency_ms"])
    assert len(result) <= len(sample_data)
    assert "download_latency_ms" in result.columns


def test_isolation_forest_filtering(sample_data):
    result = filter_outliers_isolation_forest(
        sample_data, 0.75, ["download_latency_ms"]
    )
    assert len(result) <= len(sample_data)
    assert "download_latency_ms" in result.columns
